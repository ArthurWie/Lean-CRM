...
- **Look & feel**: Clean, focused, not playful — **Twenty CRM design system** (LOCKED 2026-06-23, supersedes direction B): adopts the open-source Twenty design tokens delivered as CSS variables — 12-step grayscale, indigo accent `#3e63dd` (replaces violet `#6366f1`), radii 2/4/8/20/pill, a 4px spacing base, and gray-alpha shadows. **Inter + DM Mono are self-hosted offline** (vendored woff2 + `@font-face`, no CDN), and all navigational/action icons are real `@tabler/icons-react` components. **Sole carryover from the superseded direction B: the 🔥 flame stays a small, subtle emoji** — every other icon is a Tabler component. The locked source of truth is now `.planning/phases/07-twenty-style-redesign-adopt-the-twenty-crm-design-system-tok/07-UI-SPEC.md` (with the live app). **Superseded:** direction B (Linear/SaaS, violet `#6366f1`, quick task `260619-epx`), the `01-*/01-UI-SPEC.md` design language with `design-explorations.html` theme B, and the older navy/2px `lean-crm-mockup.html`.
